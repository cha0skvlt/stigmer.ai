...
| **Stigmergy (swarm)** | `X-Agent-Key` (per-agent bcrypt) or human key on task routes | `/api/tasks/*`, optimistic `version` on cards | [`backend/store.py`](backend/store.py), [`backend/task_api.py`](backend/task_api.py), [`docs/AGENTS.md`](docs/AGENTS.md) |
...
| Agent playbook | Full loop and examples | [`docs/AGENTS.md`](docs/AGENTS.md) |
